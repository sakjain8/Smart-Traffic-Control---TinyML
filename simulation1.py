...
def load_vehicle_data(file_path):
    ...
    for _, row in df.iterrows():
        # Extract numerical signal number from 'Redlight 1'
        match = re.search(r'\d+', row['Signal'])
        if not match:
            continue  # Skip if no valid signal number found
        signal = int(match.group())  

        # Extract numeric part from image filename (e.g., '1.jpg' -> 1)
        image_match = re.search(r'\d+', row['Image'])
        if not image_match:
            continue  # Skip if no valid image number found
        image_round = int(image_match.group())

        if (signal, image_round) not in vehicle_data:
            vehicle_data[(signal, image_round)] = {
                'Car': int(row['Car']),
...
# Provide the correct CSV file path
vehicle_data = load_vehicle_data("C:\\Users\\saksh\\Desktop\\project\\outputs\\vehicle_counts.csv")
...
def setTime():
    ...
    global currentRound, currentGreen, signals, vehicle_data
    global carTime, motorcycleTime, busTime, truckTime
    global noOfLanes, defaultMinimum, defaultMaximum

    logging.debug(f"time for current green signal: {currentGreen}")
    nextGreen = (currentGreen + 1) % len(signals)
    
    logging.debug(f"Setting time for next green signal: {nextGreen}")
    key = (nextGreen + 1, currentRound)  # Signal numbers are 1-based

    if key in vehicle_data:
        vehicle_counts = vehicle_data[key]
        
        # Calculate weighted sum
        weighted_sum = (
            vehicle_counts.get('Car', 0) * carTime +
            vehicle_counts.get('Motorcycle', 0) * motorcycleTime +
            vehicle_counts.get('Bus', 0) * busTime +
            vehicle_counts.get('Truck', 0) * truckTime
        )
        
        # Calculate green time
        greenTime = math.ceil(weighted_sum / (noOfLanes + 1))
        
        # Ensure green time is within bounds
        greenTime = max(defaultMinimum, min(greenTime, defaultMaximum))
        
        signals[nextGreen].green = greenTime
    else:
        # If no data for this round and signal, use default minimum
        signals[nextGreen].green = defaultMinimum
    
    # Set red time for other signals
    for i in range(len(signals)):
        if i != nextGreen:
            signals[i].red = signals[nextGreen].green + signals[nextGreen].yellow
    
    print(f"Set green time for signal {nextGreen + 1}: {signals[nextGreen].green}")
...
def updateValues():
    for i in range(0, noOfSignals):
        if(i==currentGreen):
            if(currentYellow==0):
                signals[i].green-=1
                logging.debug(f"Signal {i+1} green time: {signals[i].green}")
...
def generateVehicles():
    global currentRound
    currentRound += 1  
    logging.debug(f"Generating vehicles for round {currentRound}")
    if currentRound>5:
        currentRound=0
    for signal in range(noOfSignals):
        key = (signal + 1, currentRound)  # Signal numbers are 1-based
        if key in vehicle_data:
            logging.debug(f"Vehicles for signal {signal + 1}: {vehicle_data[key]}")
            vehicle_counts = vehicle_data[key]
            for vehicle_type, count in vehicle_counts.items():
                for _ in range(count):
                    lane_number = 0 if vehicle_type == 'Motorcycle' else random.randint(1, 2)
                    will_turn = 1 if random.random() < 0.3 else 0
                    Vehicle(lane_number, vehicle_type.lower(), signal, directionNumbers[signal], will_turn)
    nextGenerationTime = min(signals[currentGreen].green+10, 30)
    threading.Timer(nextGenerationTime, generateVehicles).start()
